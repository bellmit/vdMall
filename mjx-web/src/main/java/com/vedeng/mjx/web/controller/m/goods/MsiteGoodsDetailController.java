package com.vedeng.mjx.web.controller.m.goods;
...
@RestController
public class MsiteGoodsDetailController extends BaseController {
    ...
    public ResultInfo queryGoodsInfo(@RequestBody RequestParameter parameter) {
        ...
        //校验skuNo
        String skuNo = parameter.getSkuNo();
        ...
        if (getAccountId() == null) {
            String backUrlStr = contextUrl + "/goods/" + skuNo + "/sale-raiders.html";
            Map backUrl = new HashMap<String, String>();
            backUrl.put("backUrl", backUrlStr);
            return ResultInfo.fail(VedengErrorCode.USER_NOT_LOGIN_YET, backUrl);
        }
        //没权限 转发到首页
        if (!checkPriceAuth()) {
            return ResultInfo.fail(VedengErrorCode.NO_PRICE_AUTH);
        }
